Rails.application.routes.draw do
  devise_for :users
  resources :users do
    member do
      get :following, :followers
    end
  end
  #resources :contact
  root :to => 'home#home'
  get '/subscriptions/list', to: 'subscriptions#list'
  get '/subscriptions/delete', to: 'subscriptions#delete'
  get '/subscriptions/add', to: 'subscriptions#add'
  get '/subscriptions/ratelist', to: 'subscriptions#ratelist'
  post '/subscriptions/delete', to: 'subscriptions#delete'
  post '/subscriptions/add', to: 'subscriptions#add'
  get '/feeds/list', to: 'feeds#list'
  get '/feeds/create', to: 'feeds#create'
  get '/feeds/save', to: 'feeds#save'
  post '/feeds/save', to: 'feeds#save'
  post '/feeds/search', to: 'feeds#search'
  get '/feeds/search_input', to: 'feeds#search_input'
  resources :relationships,       only: [:create, :destroy]

  #match ':controller(/:action(/:id(.:format)))', :via => :all

  # The priority is based upon order of creation: first created -> highest priority.
  # See how all your routes lay out with "rake routes".

  # You can have the root of your site routed with "root"
  # root 'welcome#index'

  # Example of regular route:
  #   get 'products/:id' => 'catalog#view'

  # Example of named route that can be invoked with purchase_url(id: product.id)
  #   get 'products/:id/purchase' => 'catalog#purchase', as: :purchase

  # Example resource route (maps HTTP verbs to controller actions automatically):
  #   resources :products

  # Example resource route with options:
  #   resources :products do
  #     member do
  #       get 'short'
  #       post 'toggle'
  #     end
  #
  #     collection do
  #       get 'sold'
  #     end
  #   end

  # Example resource route with sub-resources:
  #   resources :products do
  #     resources :comments, :sales
  #     resource :seller
  #   end

  # Example resource route with more complex sub-resources:
  #   resources :products do
  #     resources :comments
  #     resources :sales do
  #       get 'recent', on: :collection
  #     end
  #   end

  # Example resource route with concerns:
  #   concern :toggleable do
  #     post 'toggle'
  #   end
  #   resources :posts, concerns: :toggleable
  #   resources :photos, concerns: :toggleable

  # Example resource route within a namespace:
  #   namespace :admin do
  #     # Directs /admin/products/* to Admin::ProductsController
  #     # (app/controllers/admin/products_controller.rb)
  #     resources :products
  #   end
end
